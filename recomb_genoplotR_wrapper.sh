...
cd mafft_genoplotr/
perl convert_fasta2phylip.pl mafft_output.fa
...
mkdir -p PROKKA
python python_multiple_alignment_genopltR.py mafft_output.fa
...
for f in `ls PROKKA/*fa`; do
  ...
  nice -n 19 prokka --force --genus Staphylococcus --species "not applicable" -strain $bsname --outdir PROKKA/prokka_out/prokka_$bsname --prefix $bsname $f --locustag $bsname --addgenes
done 
...
Rscript Rscript_genoplotR_wrapper.R PROKKA/prokka_out/ RAxML_bipartitions.recomb_mafft.raxml 
